...
dateseq = pd.read_sql("select TRADE_DT from aindexeodprices\
                                ...
                                and TRADE_DT<='%s' order by TRADE_DT;" % (stadate,nowDate), dbwind)
...
stadate = '20170101'
totaldata = pd.read_sql("select TRADE_DT from aindexeodprices\
                        where S_INFO_WINDCODE ='000300.SH' and TRADE_DT>='%s'\
                        and TRADE_DT <='%s'\
                        order by TRADE_DT;" % (stadate,nowDate), dbwind)
totaldata.columns = ['Date']
for ii in ['A','B','C','D']:
    idxdata = pd.read_sql("select TRADE_DT,S_DQ_CLOSE from %s where \
                              S_INFO_WINDCODE='%s' and TRADE_DT >= '%s' \
                              and TRADE_DT <= '%s' \
                              order by TRADE_DT " % 
                              (assetindex[ii][1],assetindex[ii][0],stadate,nowDate), dbwind)
    idxdata.columns = ['Date',assetindex[ii][0]]
    totaldata = pd.merge(totaldata,idxdata,how='left')
for ii in ['E']:
    idxdata = pd.read_sql("select PRICE_DATE,F_NAV_ADJUSTED from chinamutualfundnav where \
                              F_INFO_WINDCODE ='%s' and PRICE_DATE>='%s'\
                              and PRICE_DATE<='%s'\
                              order by PRICE_DATE " % 
                              (assetindex[ii][0],stadate,nowDate), dbwind)
    idxdata.columns = ['Date',assetindex[ii][1]]
    totaldata = pd.merge(totaldata,idxdata,how='left')
totaldata = totaldata.fillna(method='pad').fillna(method='bfill')
...
#    trueStaDate = datetime.datetime.strptime(tradedate,'%Y%m%d') + datetime.timedelta(days = -500)
#    trueStaDate = trueStaDate.strftime('%Y%m%d')
    ...
    assetReturn = assetpct[tracestr:tradedatestr].mean()*240 # historical return
...
#        code = ptfCode[15]
        ...
        for ii in range(0,len(riskClasses)):
            ...
            riskClassNums.append(riskClassList[riskcode][1][ii])
...
#        PI = matrix(assetReturn)
        ...
        cnt=0
        prest = 100 - sum(w_hat) # 有时四舍五入后，权重和不足1，须调整权重
        ...
        for ii in range(0,len(riskClasses)):
            ...
            assetn = riskClassNums[ii]
            if assetc in ['C','D','E']:
                assetc += str(p1)
            sql = "select fund_code, fund_name from stg_fund_info where class = '%s' limit 0,%d;" % (assetc,assetn)
            cursor.execute(sql)
            funds = cursor.fetchall()
            for jj in range(0,assetn):
                fundtext += funds[jj][1] +' (' + funds[jj][0] + ') ' + str(w_hat[ii]/assetn) + '%, '
                fundtext2 += funds[jj][0] + '-' + str(w_hat[ii]/assetn) + '/'
...
#            nowDate = (datetime.datetime.now() + datetime.timedelta(-100)).strftime('%Y%m%d')
        ...
        sta = 0
        if tradedate == shiftDate['TRADE_DT'][-1]:
            sta = 1
        datatuple = (tradedate,code,fundtext,'1','定期BL模型资产配置',sta,nowTime,nowTime) 
        datatuple2 = (tradedate,code,fundtext2,'1','定期BL模型资产配置',sta,nowTime,nowTime) 
        # 插入带基金简称的基金列表
        sql = "INSERT INTO `stg_portfolio_info_more`(\
        `create_date`, `code`, `fund_list`, `type`, \
        `change_reason`, `status`, `create_time`, `update_time`) \
        select '%s', '%s', '%s', '%s', '%s', '%d', '%s', '%s' from dual " % datatuple + "where \
        not exists (select * from stg_portfolio_info_more where `create_date`='%s'\
        and `code` = '%s')" % datatuple[:2]
        cursor.execute(sql)
        
        # 插入简化的基金列表
        sql = "INSERT INTO `stg_portfolio_info`(\
        `create_date`, `code`, `fund_list`, `type`, \
        `change_reason`, `status`, `create_time`, `update_time`) \
        select '%s', '%s', '%s', '%s', '%s', '%d', '%s', '%s' from dual " % datatuple2 + "where \
        not exists (select * from stg_portfolio_info where `create_date`='%s'\
        and `code` = '%s')" % datatuple2[:2]
        cursor.execute(sql)
...
resultdf.to_csv(r".\Data\allocation.csv",index = False) # 如果要存csv，需要先在文件开头修改工作目录
